...
int safety(int process) {//are we in a safe state?
	...
	for(int i=0; i<NUMBER_OF_CUSTOMERS; i++) {
 		for(int j=0; j<NUMBER_OF_RESOURCES; j++) {//test resources with customers
...
int main(int argumentCount, char* argumentArray[]) {
	printf("Available\n{ ");
 	for(int i=1; i<argumentCount; i++) {   //create argument array
	...
	pthread_mutex_init(&mutex, NULL);//initialize mutex
 	for(int i=0; i<NUMBER_OF_CUSTOMERS; i++) {//create multiple threads
		pthread_create(&Customer[i], &attr, create, NULL);
	}
	for(int i=0; i<NUMBER_OF_CUSTOMERS; i++) {//join threads
